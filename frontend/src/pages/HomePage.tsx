...
import { Sparkles, Loader2, ChevronRight, History, Paperclip, X, FileText, ImageIcon } from 'lucide-react';
...
const HomePage: React.FC<HomePageProps> = ({ onOpenAuth }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // We do NOT clear topic here anymore, allowing users to add instructions
      setError(null);
    }
  };
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        // File Upload Flow with optional topic instructions
        data = await uploadFileAndGenerate(selectedFile, topic, (status) => {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleGenerate} className="relative max-w-lg mx-auto mb-16 group z-10">
        ...
        <div className="relative flex items-center bg-white dark:bg-slate-900 rounded-2xl shadow-xl shadow-slate-200/40 dark:shadow-slate-900/50 border-2 border-slate-100 dark:border-slate-800 p-1">
          ...
          {/* Combined Input Area: File Chip + Text Input */}
          <div className="flex-grow flex items-center pl-2">
            <AnimatePresence>
              {selectedFile && (
                <motion.div 
                  ...
                  className="flex items-center gap-1.5 bg-indigo-50 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 px-2 py-1.5 rounded-lg mr-2 shrink-0 max-w-[150px]"
                >
                  {isImageFile(selectedFile) ? <ImageIcon className="w-4 h-4 shrink-0" /> : <FileText className="w-4 h-4 shrink-0" />}
                  <span className="text-xs font-medium truncate">{selectedFile.name}</span>
                  <button
                    type="button"
                    onClick={clearFile}
                    className="ml-1 p-0.5 hover:bg-indigo-100 dark:hover:bg-indigo-800 rounded-full transition-colors"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </motion.div>
            ...
            <input
              ...
              placeholder={selectedFile ? "Add instructions (e.g., 'Extract notes')..." : "Enter a topic..."}
              className="w-full h-full py-3 bg-transparent focus:outline-none text-lg placeholder:text-slate-400 dark:placeholder:text-slate-600 text-slate-800 dark:text-slate-100 min-w-[50px]"
              disabled={isLoading}
            />
          </div>
          ...
          <div className="flex items-center gap-1 pr-1 shrink-0">
            {!selectedFile && (
              <button
                ...
                className="p-2.5 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-xl transition-colors"
            ...
            <button
              ...
              className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-600 text-white rounded-xl px-4 py-2 font-medium transition-colors flex items-center justify-center h-10 shadow-sm"
            >
              {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <ChevronRight className="w-6 h-6" />}
            </button>
          </div>
        </div>
        {!selectedFile && <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">Or click the paperclip to upload a PDF, TXT, or Image (OCR)</p>}
      </form>
