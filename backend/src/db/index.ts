...
export const saveStudySet = async (data: StudySet, embedding?: number[]) => {
  if (!process.env.DATABASE_URL) {
...
export const findSimilarStudySet = async (embedding: number[]): Promise<StudySet | null> => {
  if (!process.env.DATABASE_URL) return null;
  ...
  try {
    ...
    if (res.rows.length > 0) {
      ...
      if (bestMatch.distance < 0.25) {
         ...
         return getStudySetById(bestMatch.id);
      } else {
